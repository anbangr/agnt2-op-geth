...
func TestValidateAGNT2TypedReexecFields(t *testing.T) {
	...
	root, count := types.FoldTypedReexecRoot(txs, signer, nil)
	require.EqualValues(t, 1, count)

	// Correct commitment validates.
	good := &types.Header{TypedReexecRoot: &root, TypedReexecCount: &count}
	require.NoError(t, validateAGNT2TypedReexecFields(good, txs, signer, nil))

	// Absent pair is a clean no-op ONLY when the block has no foldable ops.
	require.NoError(t, validateAGNT2TypedReexecFields(&types.Header{}, []*types.Transaction{}, signer, nil))
	...
	require.Error(t, validateAGNT2TypedReexecFields(&types.Header{}, txs, signer, nil))

	// Tampered root is rejected.
	bad := root
	bad[0] ^= 0xFF
	require.Error(t, validateAGNT2TypedReexecFields(&types.Header{TypedReexecRoot: &bad, TypedReexecCount: &count}, txs, signer, nil))

	// Half-pair (root set, count nil) is rejected.
	require.Error(t, validateAGNT2TypedReexecFields(&types.Header{TypedReexecRoot: &root}, txs, signer, nil))

	// Wrong count is rejected.
	wrongCount := uint64(2)
	require.Error(t, validateAGNT2TypedReexecFields(&types.Header{TypedReexecRoot: &root, TypedReexecCount: &wrongCount}, txs, signer, nil))
}
